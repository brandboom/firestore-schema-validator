...
class Model {
  ...
  constructor (_snapshot, _data) {
    ...
    if (!(_snapshot instanceof SDK.firebase.firestore.DocumentSnapshot))
   ...
   * @memberof Model
   */
  static async create(data = {}, id=null) {
    let snapshot;
    if(id)
      snapshot = await this._collectionRef.doc(id).get()
    else
      snapshot = await this._collectionRef.doc().get()
